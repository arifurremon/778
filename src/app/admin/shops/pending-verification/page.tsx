  ...
  const handleAction = async (id: string, action: 'approve' | 'reject') => {
    ...
    try {
      ...
      toast({ 
        ...
        description: `Shop has been ${action === 'approve' ? 'verified' : 'rejected'}.` 
      });
  ...
  return (
    <div className="p-6 md:p-8 space-y-8 max-w-5xl mx-auto">
      <div>
        ...
        <h1 className="text-3xl font-black tracking-tight italic">Shop Verification</h1>
        <p className="text-sm text-muted-foreground mt-1">Review new shop registration requests</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 bg-muted/5 rounded-3xl border-2 border-dashed border-border/50">
          ...
          <p className="text-sm text-muted-foreground mt-1 font-medium">All shop requests have been processed.</p>
        </div>
      ) : (
        <div className="grid gap-6">
          <AnimatePresence>
              ...
              >
                <Card className="overflow-hidden border-border/50 shadow-xl shadow-black/5 hover:border-border transition-all">
                  <CardHeader className="flex flex-row items-start justify-between gap-4 pb-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex flex-col">
                        <CardTitle className="text-xl font-black tracking-tight">{shop.name}</CardTitle>
                        <div className="flex items-center gap-2 text-[10px] text-muted-foreground font-bold uppercase tracking-widest">
                          <Clock size={10} /> Requested {format(new Date(shop.createdAt), 'PPp')}
                        </div>
                      </div>
                    </div>
                  </CardHeader>
                  ...
                  <CardContent className="grid md:grid-cols-3 gap-6">
                    <div className="md:col-span-2 space-y-4">
                      <div className="space-y-1">
                        <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Shop Description</Label>
                    ...
                    <div className="bg-muted/30 rounded-2xl p-4 border border-border/30 space-y-4">
                      <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground block mb-2">Owner Identity</Label>
                      <div className="flex items-center gap-3">
                        <Avatar className="w-10 h-10 ring-2 ring-background">
                          <AvatarImage src={shop.owner.profileImage || ""} />
                          <AvatarFallback className="font-bold">{shop.owner.name[0]}</AvatarFallback>
                        </Avatar>
                        <div className="flex flex-col">
                          <span className="text-xs font-bold">{shop.owner.name}</span>
                          <span className="text-[10px] text-muted-foreground truncate max-w-[120px]">{shop.owner.email}</span>
                        </div>
                      </div>
                  ...
                  <CardFooter className="bg-muted/10 border-t border-border/30 flex items-center justify-end gap-2 p-3">
                    <Button 
                      variant="outline" 
                      onClick={() => setRejectDialog({ open: true, id: shop.id, reason: "" })}
                    ...
                    >
                      {actionLoading === shop.id ? <Clock className="h-4 w-4 animate-spin" /> : <ShieldCheck size={14} />}
                      Approve Shop
                    </Button>
                  </CardFooter>
                </Card>
        ...
        <DialogContent className="max-w-md rounded-3xl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-rose-600 italic">
              <AlertCircle size={20} /> Reject Shop Verification
            </DialogTitle>
            <DialogDescription>
              Please provide a detailed reason for rejection. This will be sent to the shop owner.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-2 py-4">
            ...
            <Textarea 
              id="reason"
              placeholder="e.g., The provided address documentation is incomplete or incorrect..."
